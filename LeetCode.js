...
var sumOfEncryptedInt = function(nums) {
   var sum=0
for(let i=0;i<nums.length;i++){
   var temp=nums[i].toString().split('').map(Number)
 var max=Math.max(...temp).toString()
 var num=max.repeat(temp.length)
 sum+=parseInt(num)
...
var isSumEqual = function(firstWord, secondWord, targetWord) {
   var first=firstWord.split('').map((val)=>val.charCodeAt()-97).join('')
var second=secondWord.split('').map((val)=>val.charCodeAt()-97).join('')
var third=targetWord.split('').map((val)=>val.charCodeAt()-97).join('')
